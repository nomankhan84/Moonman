...
// Assume user is logged in and user id is stored in $_SESSION['user_id']
$user_id = $_SESSION['user_id'] ?? 0;

// Check if the user has already submitted bank details
$bankExists = false;
$check = $conn->prepare("SELECT id FROM user_bank_details WHERE user_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !$bankExists) {
    $account_number = trim($_POST['account_number']);
    $ifsc_code = trim($_POST['ifsc_code']);
    $upi_id = trim($_POST['upi_id']);

    if ($account_number && $ifsc_code) {
        $stmt = $conn->prepare("INSERT INTO user_bank_details (user_id, account_number, ifsc_code, upi_id) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("isss", $user_id, $account_number, $ifsc_code, $upi_id);
        if ($stmt->execute()) {
            $_SESSION['success'] = "Bank details added successfully.";
            header("Location: bind-bank.php");
            exit;
        } else {
...
>
  ...
  <body>
    ...
    <h2>Bind Your Bank Account / UPI</h2>
    ...
    <?php if (!$bankExists): ?>
        <form method="POST">
            ...
            <div class="alert alert-warning">
                Note: Please make sure the bank account entered is linked with the mobile number you used to register.
            </div>
    ...
    <?php else: ?>
        <div class="alert alert-info">
            You have already added your bank details.
        </div>
    <?php endif; ?>
</div>
    </div>
