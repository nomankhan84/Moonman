<?php
include 'db_conn.php'; // mysqli $conn

// 1) auto-deactivate expired notices
$today = date('Y-m-d');
$conn->query("
    UPDATE notices
    SET status = 'inactive'
...
// 2) handle add / deactivate / delete actions
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_notice'])) {
    $h = $conn->real_escape_string($_POST['heading']);
    $b = $conn->real_escape_string($_POST['body']);
    $e = $conn->real_escape_string($_POST['end_date']);
    $conn->query("
        INSERT INTO notices (heading, body, end_date)
        VALUES ('{$h}','{$b}','{$e}')
    ");
...
if (isset($_GET['action'], $_GET['id'])) {
    ...
    if ($_GET['action'] === 'deactivate') {
        $conn->query("UPDATE notices SET status='inactive' WHERE id={$id}");
    }
    ...
    <table class="table">
      ...
      <tbody class="table-border-bottom-0">
        <?php foreach ($notices as $n): 
          $badge = $n['status']==='active' ? 'bg-label-success' : 'bg-label-secondary';
        ?>
        <tr>
          <td><strong><?= htmlspecialchars($n['heading']) ?></strong></td>
          ...
          <td>
            <div class="dropdown">
              ...
              <div class="dropdown-menu">
                ...
                <a 
                  class="dropdown-item"
                  href="?action=deactivate&id=<?= $n['id'] ?>"
                >
