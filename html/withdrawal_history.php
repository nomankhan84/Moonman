...
$stmt = $conn->prepare("SELECT amount, status, payment_method, created_at FROM withdrawal_requests WHERE user_id = ? ORDER BY created_at DESC");
$stmt->bind_param("i", $user_id);
$stmt->execute();
                ...
                <table class="table">
                  ...
                  <tbody class="table-border-bottom-0">
                    <?php foreach ($data as $row): ?>
                      <tr>
                        <td><i class="fas fa-arrow-up text-danger me-2"></i><strong>Withdrawal</strong></td>
                        <td>₹<?= number_format($row['amount'], 2) ?></td>
                        <td>
                          <span class="badge <?= $row['status'] === 'completed' ? 'bg-label-success' : 'bg-label-warning' ?> me-1">
